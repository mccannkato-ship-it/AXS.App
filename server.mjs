...
async function ticketmaster(url) { const params = new URLSearchParams(url.searchParams); const apiKey = process.env.Ticketmaster_API || process.env.TICKETMASTER_API_KEY; if (apiKey) params.set('apikey', apiKey); params.set('size', params.get('size') || '24'); const upstream = await fetch(`https://app.ticketmaster.com/discovery/v2/events.json?${params}`); return new Response(await upstream.text(), {status:upstream.status, headers:{'content-type':'application/json; charset=utf-8','cache-control':'public, max-age=60'}}) }
