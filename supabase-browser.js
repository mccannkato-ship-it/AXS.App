...
export async function signUpWithSupabase(email, password) { if (!supabase) return { data: null, error: new Error('Supabase Auth is not configured.') }; return supabase.auth.signUp({ email, password, options: { emailRedirectTo: `${window.location.origin}/auth/callback` } }) }
